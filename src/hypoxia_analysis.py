...
from data_loading import scan_dir, parse_hypoxia_csv
...
def main():
	data_dir = '../data/example/O2-curves'
	output_dir = '../run/output'
	if not path.isdir(output_dir):
		os.makedirs(output_dir)
	for data_file in scan_dir(data_dir, regex=r'.*/Ex\-USA_.*_\(rep \d+\)\.csv'):
		print('Processing "%s"...' % data_file)
		_, title, subtitle = data_file.replace('.csv', '').split('_')
		hypoxia_data = parse_hypoxia_csv(data_file).replace(nan, None).dropna()
		...
		analyze_model(
			...
			figfile=path.join(output_dir, 'AIC-selection_Hughes-Data_%s_%s.pdf' % (title, subtitle)), show=True
		)
	print('...Done!')
